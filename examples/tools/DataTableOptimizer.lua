...
if arg and arg[1] then
	...
	ClientRoot = "D:\\program\\Project3\\Client\\Trunk"
end
local DatabaseRoot = ""
if arg and arg[2] then
	DatabaseRoot = arg[2]
else
	DatabaseRoot = "D:\\program\\Project3\\Tools\\luatabletemp"
end
...
ClientRoot = string.gsub( ClientRoot, '\\', '/' )
DatabaseRoot = string.gsub( DatabaseRoot, '\\', '/' )

local LuaRoot = ClientRoot .. "/LuaProject/LuaScripts"
local TablePreProcess = ClientRoot .. "/LuaProject/TablePreProcess"
-- package.path = package.path..';'..DatabaseRoot..'/?.lua'..';'.. LuaRoot..'/?.lua'..';'.. TablePreProcess..'/?.lua'
package.path = package.path..';'.. LuaRoot ..'/?.lua'
...
local DefaultNumberSerializedFormat = "%.17g"
local NumberSerializedFormat = DefaultNumberSerializedFormat
local DatabaseLocaleTextName = "locale_text"
...
local preprocessTableHead = "preprocess_"
local postprocessTableFile = "global_postprocess" 
local TableCloseDefaultName = "__closedefaultvalue"
local TableNoOptimizeName = "__rawdata"
local TableIsDoubleKeyName = "__isdoublekeydata"
local TableEnableNewIndex = "__enableNewIndex"
		...
		if nt == "string" then
			if name ~= "" then
				if string.match( name,'^%d+' ) then
					...
					append( name )
					append( "\"]" )
				elseif string.match(name,'[^%w_]+') then
					append( "[\"" )
					append( name )
					append( "\"]" )
				else
...
local function SaveStringBankToLua( stringBank, tofile )
	if tofile then
		...
		OrderedForeach(stringBank,function(k,v)
...
local function GetAllFileNamesAtPath( path )
	...
	if plat == "windows" then
		...
		for dir in io.popen( string.format( "find \"%s\" -not -type d", path ) ):lines() do
			local s, e, f = dir:find( ".+/(.+)%.lua$" )
			if f then
				table.insert( ret, f )
			end
...
local function GetLuaFileName(name)
	...
	local fileName = DatabaseRoot.."/".. pname ..".lua"
	return fileName
end

local function LoadPreProcess(name)
	local curName = preprocessTableHead .. name ..".lua"
	local fileName = TablePreProcess.."/"..curName
...
local function LoadDataset( name )
	...
	local loader = function( name )
		local r = Database[name]
		if r then
			return r
		end

		local fileName = GetLuaFileName(name)
		...
		--table预处理
		local preFunc = LoadPreProcess( name )
		if preFunc then
			print("preprocess Data:" .. name)
			rval = preFunc(rval)
		end
...
local function PostProcessTable(name)
	local fileName = TablePreProcess.."/".. postprocessTableFile .. ".lua"
...
local function OptimizeDataset( dataset )
	local isCloseDefaultValueOptimize = dataset[TableCloseDefaultName] or false
	dataset[TableCloseDefaultName] = nil
	...
	for k, v in pairs( dataset ) do
		...
		if _sk ~= "__name" and _sk ~= "__sourcefile" and _sk ~= TableCloseDefaultName then
			if not idType then
				idType = type( k )
			end
	...
	if EnableDefaultValueOptimize and not isCloseDefaultValueOptimize then
					...
					if count >= max then
						if count > max then
							...
							if curType == "table" then
								...
								local _value = value
								_value = SerializeTable(_value, true, true)
...
local function OptimizeDatasetDoubleKey( dataset )
	local isCloseDefaultValueOptimize = dataset[TableCloseDefaultName] or false
	dataset[TableCloseDefaultName] = nil
	...
	for k, v in pairs( dataset ) do
		...
		if _sk ~= "__name" and _sk ~= "__sourcefile" and _sk ~= TableCloseDefaultName then
			...
			for k1,v1 in pairs(v) do
				...
				if idType == type( k1 ) then
					tmp_ids[ #tmp_ids + 1 ] = k1
				end
			end
			ids[k] = tmp_ids
		end
	...
	if EnableDefaultValueOptimize and not isCloseDefaultValueOptimize then
		-- find bigest table to generate all fields
		local majorItemKey1 = 1
		local majorItemKey2 = 1
		local f = 0
		for k, v in pairs( dataset ) do
			if type( v ) == "table" then
				for k1,v1 in pairs(v) do
		...
		local v = dataset[ majorItemKey1 ] ~= nil and dataset[ majorItemKey1 ][majorItemKey2] or nil
		if type( v ) == "table" then
			for name, value in pairs( v ) do
				local nt = type( name )
				-- if nt == "string" and name == "id" then
			...
			for i, field in ipairs( names ) do
				-- for all record / row
				for r, t in pairs( ids ) do
					for r1,t1 in ipairs(t) do
						local record = dataset[ r ][ t1 ]
						if record[ field ] ~= nil then
							local v = record[ field ]
							local curType = GetValueTypeNameCS( v )
							if not typeNameTable[ field ] then
				...
				OrderedForeach(
					...
					function(k1,v1)
						...
						if idType == type( k1 ) then
							...
							if idType == "number" then
								if isIntegerKey then
									isIntegerKey = k1 == floor( k1 )
								end
							end
						end

						if type( v1 ) == "table" then
							overwrites[k] = overwrites[k] or {}
							overwrites[ k ][k1] = UniquifyTable( v1 )
						end
	...
	if overwrites then
		for k, v in pairs( overwrites ) do
			for k1,v1 in pairs(overwrites) do
				dataset[ k ][ k1 ] = overwrites[ k ][ k1 ]
			end
...
local function SaveDatasetToFile( dataset, tofile, tableRef, name )
	...
	--remove not need value
	dataset["__name"] = nil
	dataset["__sourcefile"] = nil
...
local function ExportOptimizedDataset( t, StringBank )
	...
	local tableRef = nil
	local defaultValues = nil
	local noOptimizeRawData = nil
	local isDoubleKeyData = false
	local enableNewIndex = false
	if EnableDatasetOptimize then
		enableNewIndex = t[TableEnableNewIndex] or false
		t[TableEnableNewIndex] = nil
		noOptimizeRawData = t[TableNoOptimizeName]
		t[TableNoOptimizeName] = nil
		isDoubleKeyData = t[TableIsDoubleKeyName] or false
		t[TableIsDoubleKeyName] = nil
		if isDoubleKeyData then
			--优化双key的结构
			defaultValues = OptimizeDatasetDoubleKey( t )
		else
		...
		if defaultValues then
			local removeDefaultValues = function( record )
				...
				for field, defaultVal in pairs( defaultValues ) do
					...
					if hasValue and type(value) == "table" and type(defaultVal) == "table" then 
						if SerializeTable( value, true, true ) == SerializeTable( defaultVal, true, true ) then
							needRemove = true
						end
					elseif value == defaultVal and hasValue then
						needRemove = true
					end
			...
			local removed = {}
			if isDoubleKeyData then
				for kName, record in pairs( t ) do
					if type( record ) == "table" then
						for kName1 , record1 in pairs( record ) do
							if type( record1 ) == "table" then
								if not removed[ record1 ] then
									removeDefaultValues( record1 )
									removed[ record1 ] = true
								end
							end
						end
					end
		...
		tableRef = {
			...
			postOutput = function( outFile )
				...
				--默认不优化的数据
				if type(noOptimizeRawData) == "table" or defaultValues then
					local __rawdata = type(noOptimizeRawData) == "table" and noOptimizeRawData or {}
					__rawdata.__basemetatable = defaultValues
					local rawdataRef = { [defaultValues] = "base" }
					outFile.write( string.format( "local __rawdata = %s\n", SerializeTable( __rawdata, false, false,rawdataRef ) ) )
					outFile.write( string.format( "setmetatable(%s,{ __index = __rawdata })\n", datasetName ) )
				end
			end
		}
	end
...
local function ExportDatabaseLocalText( tofile, newStringBank )
	...
	for _,v in ipairs(files) do
		if not ExcludedFiles[ v ] then
			local t = Database[ v ]
			local localized = false
			local sourcefile = GetLuaFileName(v)
			if t then
				t.__name = v
				t.__sourcefile = sourcefile
